...
class AssetService:
    ...
    def _build_object_key(self, media_type: str, dataset_name: Optional[str], filename: str) -> str:
        ...
        path_prefix = f"{prefix}/{datetime.now():%Y/%m/%d}/"
        uuid_prefix = f"{py_uuid.uuid4().hex}_"
        file_path = Path(filename)
        filename_only = file_path.name
        suffix = f"{uuid_prefix}{filename_only}"
        max_suffix_length = MAX_OBJECT_KEY_LENGTH - len(path_prefix)

        if max_suffix_length <= 0:
            raise HTTPException(status_code=400, detail='dataset_name is too long to build a valid object key')

        if len(suffix) > max_suffix_length:
            allowed_stem_length = max_suffix_length - len(uuid_prefix) - len(file_path.suffix)
            if allowed_stem_length <= 0:
                raise HTTPException(status_code=400, detail='filename is too long to build a valid object key')
            suffix = f"{uuid_prefix}{file_path.stem[:allowed_stem_length]}{file_path.suffix}"
    ...
    def sync_assets_from_minio_prefix(
        self,
        db: Session,
        bucket_name: str,
        object_prefix: str,
        ...
        if not prefix:
            raise HTTPException(status_code=400, detail='object_prefix is required')

        normalized_prefix = f'{prefix}/'
        path_parts = prefix.split('/')
        dataset_name = path_parts[0] if path_parts else None
        sequence_name = path_parts[1] if len(path_parts) > 1 else None

        existing_stmt = select(MediaAsset).where(
            MediaAsset.bucket_name == bucket_name,
            MediaAsset.object_key.like(f'{normalized_prefix}%'),
        )
        existing_assets = list(db.scalars(existing_stmt).all())
        existing_by_key = {asset.object_key: asset for asset in existing_assets}

        created = False
        for obj in self.minio_service.list_objects(bucket_name, prefix=normalized_prefix, recursive=True):
            object_key = getattr(obj, 'object_name', '') or ''
            if not object_key or object_key.endswith('/'):
                continue
            if object_key in existing_by_key:
                continue
        ...
        final_stmt = (
            ...
            .order_by(MediaAsset.object_key.asc())
            .limit(page_size)
        )
        return list(db.scalars(final_stmt).all())

    def list_sequence_prefixes(
        self,
        bucket_name: str,
        object_prefix: str,
    ) -> list[dict]:
        ...
        for obj in self.minio_service.list_objects(bucket_name, prefix=normalized_prefix, recursive=True):
            ...
            if not object_key.startswith(normalized_prefix):
                continue
            remainder = object_key[len(normalized_prefix):].strip('/')
            if not remainder:
                continue
            sequence_name = remainder.split('/', 1)[0].strip()
            if sequence_name:
        ...
        return [
            {
                'sequence_name': sequence_name,
                'object_prefix': f'{normalized_prefix}{sequence_name}',
            }
