...
/**
 * Initialize a SHM Producer (Zero-Copy Slot-Based).
 * Creates the file if it doesn't exist, resizes it, and maps it.
 * 
 * The capacity parameter is now interpreted as SLOT SIZE (max packet size).
 * Internally uses 32 slots for zero-copy transmission.
 * 
 * Compatibility note:
 * - Matches Java `ZeroCopySharedMemory` V2 layout (cache-line aligned) and BIG-ENDIAN metadata.
 * 
 * @param path Path to the shared memory file (e.g., "/dev/shm/my_stream")
 * @param capacity Max size of a single packet (slot size in bytes)
 ...
 * @return Number of bytes read (positive), 0 if no data, or -1 if buffer too small.
 */
int32_t midware_shm_read(midware_shm_ctx_t* ctx, void* buf, int32_t max_len);

/**
 * Read the latest data from the ring buffer, dropping all older packets.
 * Only valid for Consumer.
 *
 * @param ctx Handle
 * @param buf Output buffer
 ...
 * @param type Packet Type (0=HighFreq, 1=LowFreq, 2=ImageFrame, 3=RdmaRawImage)
