...
def model_class_map(model: YOLO) -> Dict[str, int]:
...
def resolve_target_class_ids(model: YOLO, target_classes: Any) -> Optional[List[int]]:
    if not target_classes:
        return None
    if isinstance(target_classes, str):
        raw_classes = [item.strip() for item in target_classes.split(",") if item.strip()]
    else:
        raw_classes = [str(item).strip() for item in target_classes if str(item).strip()]
    if not raw_classes:
        return None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Single-image YOLO prediction entrypoint.")
...
def main() -> None:
    ...
    conf_threshold = float(args.conf if args.conf is not None else model_cfg.get("conf", 0.35))
    ...
    target_class_ids = resolve_target_class_ids(model, model_cfg.get("target_classes"))
