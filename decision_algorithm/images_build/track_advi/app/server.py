...
app = FastAPI(title="AVTrack Tracking HTTP Service")
...
def _raw_bytes_to_image_array(image_bytes: bytes, width: int, height: int, channels: int) -> tuple[np.ndarray, float]:
    if not image_bytes:
    ...
    return image_array, raw_decode_ms
...
def track_output_to_dict(out) -> dict:
    ...
    if out.timings_ms is not None:
        payload["timings_ms"] = dict(out.timings_ms)
...
@app.get("/version")
def version():
    return {"version": "track-advi-http-v1"}
...
async def _set_template_by_raw_body(
    ...
    *,
    ...
    try:
        ...
        image_array, _ = _raw_bytes_to_image_array(image_bytes, width, height, channels)
        out = set_template(
            TemplateInput(
...
async def track_by_file(image: UploadFile = File(...), return_image: bool = Form(True)):
    ...
    try:
        ...
        engine_call_ms = (time.perf_counter() - engine_started_at) * 1000.0
        serialize_started_at = time.perf_counter()
        payload = track_output_to_dict(out)
        serialize_ms = (time.perf_counter() - serialize_started_at) * 1000.0
        timings_ms = payload.setdefault("timings_ms", {})
        timings_ms["track_upload_read_ms"] = round(upload_read_ms, 3)
        timings_ms["track_engine_call_ms"] = round(engine_call_ms, 3)
        timings_ms["track_response_serialize_ms"] = round(serialize_ms, 3)
        timings_ms["track_container_handler_ms"] = round((time.perf_counter() - handler_started_at) * 1000.0, 3)
        return payload
    except HTTPException:
        raise
    except ValueError as exc:
...
async def track_by_raw(
    ...
    try:
        ...
        image_bytes = await image.read()
        upload_read_ms = (time.perf_counter() - read_started_at) * 1000.0
        image_array, raw_decode_ms = _raw_bytes_to_image_array(image_bytes, width, height, channels)
        engine_started_at = time.perf_counter()
        ...
        timings_ms = payload.setdefault("timings_ms", {})
        timings_ms["track_upload_read_ms"] = round(upload_read_ms, 3)
        timings_ms["track_raw_buffer_decode_ms"] = round(raw_decode_ms, 3)
        timings_ms["track_engine_call_ms"] = round(engine_call_ms, 3)
        timings_ms["track_response_serialize_ms"] = round(serialize_ms, 3)
        timings_ms["track_container_handler_ms"] = round((time.perf_counter() - handler_started_at) * 1000.0, 3)
        return payload
