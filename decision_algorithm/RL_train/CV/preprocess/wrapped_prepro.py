...
class ImagePreprocessingEnv(gym.Env):
    ...
    def __init__(
        ...
        if use_docker:
            raise ValueError("Pre-generated preprocessing training requires use_docker=False")
        if modal != "RGB":
            raise ValueError(f"ImagePreprocessingEnv currently supports modal='RGB' only, got {modal}")
        ...
        self.output_root = Path(output_root or agents_root / "preprocess_outputs").resolve()

        raw_models = models or ["none", "clahe", "unsharp"]
        if isinstance(raw_models, str):
            raw_models = [item.strip() for item in raw_models.split(",") if item.strip()]
        self.actions = list(raw_models)
        if not self.actions:
            raise ValueError("At least one preprocessing action is required")
    ...
    def _build_results_path(self):
        ...
        for action_name in self.actions:
            ...
            elif action_name == "clahe":
                candidate = self.results_root / "clahe" / "mid_yolov8n_f35_e15_i640" / self.split / "json"
            elif action_name == "unsharp":
                candidate = self.results_root / "unsharp" / "mid_yolov8n_f35_e15_i640" / self.split / "json"
            else:
                raise ValueError(f"Unsupported preprocessing action without known result path: {action_name}")
            if not candidate.exists():
                available = sorted(
                    str(path)
                    for path in self.results_root.glob(f"**/{action_name if action_name != 'none' else 'yolov8'}/*/json")
                )
                raise FileNotFoundError(
                    "Result directory does not exist for preprocessing action "
                    f"'{action_name}': {candidate}. "
                    f"Current split='{self.split}'. Available cached result dirs: {available}. "
                    "Please either generate cache for this split or switch config split to an existing one."
                )
            mapping[action_name] = candidate
        return mapping
    ...
    def step(self, action):
        ...
        self.action_name = self.actions[action]
        reward = self.get_reward_conf(self.action_name, self.current_frame_index)
    ...
    def state_preprocess(self, index):
        image_path = self.img_path / self.sequence[index]
    ...
    def load_detections(self, action_name, index):
        json_path = self.results_path[action_name] / f"{Path(self.sequence[index]).stem}.json"
    ...
    def get_reward_conf(self, action_name, index):
        gt = self.load_gt(index)
        dets = self.load_detections(action_name, index)
