...
#define INDEX_QUEUE_SIZE        256  // Fixed queue metadata capacity
...
midware_shm_ctx_t* midware_shm_producer_init(const char* path, uint64_t capacity) {
    ...
    // Default slot count = 32. The index queue metadata remains 256 entries.
    const uint64_t slot_count = DEFAULT_SLOT_COUNT;
    ...
    ctx->fd = open(path, O_RDWR | O_CREAT, SHM_FILE_MODE);
    if (ctx->fd < 0) {
        perror("shm open failed");
        free(ctx);
        return NULL;
    }
    if (fchmod(ctx->fd, SHM_FILE_MODE) < 0) {
    ...
    ctx->p_next_slot = (volatile uint64_t*)(ctx->map_base + NEXT_SLOT_OFFSET);
...
midware_shm_ctx_t* midware_shm_writer_attach(const char* path, uint64_t required_capacity) {
    if (!path || required_capacity == 0) return NULL;
    ...
    ssize_t n = pread(fd, header, SLOTS_OFFSET, 0);
    if (n < SLOTS_OFFSET) {
        close(fd);
        return NULL;
    }
    ...
    uint64_t version = read_u64_be_bytes(header + VERSION_OFFSET);
    ...
    uint64_t slot_count = read_u64_be_bytes(header + SLOT_COUNT_OFFSET);
    uint64_t slot_size = read_u64_be_bytes(header + SLOT_SIZE_OFFSET);

    if (magic != SHM_MAGIC ||
        version != SHM_VERSION ||
        ready != READY_YES ||
        format != FORMAT_ZEROCOPY_V2 ||
        slot_count == 0 ||
        slot_count > INDEX_QUEUE_SIZE ||
        slot_size < required_capacity) {
        close(fd);
        return NULL;
    }
    ...
    ctx->slot_count = slot_count;
    ctx->slot_size = slot_size;
    ctx->map_size = (uint64_t)st.st_size;

    ctx->map_base = (uint8_t*)mmap(NULL, ctx->map_size, PROT_READ | PROT_WRITE, MAP_SHARED, ctx->fd, 0);
    if (ctx->map_base == MAP_FAILED) {
        close(ctx->fd);
        free(ctx);
        return NULL;
    ...
    ctx->next_free_slot = (int)(head % ctx->slot_count);
    return ctx;
}
...
bool midware_shm_write(midware_shm_ctx_t* ctx, const void* data, int32_t len) {
    ...
    // Queue full -> overwrite oldest (match Java behavior). The queue depth is
    // limited by slot_count so an unread entry never points to a reused slot.
    if (head - tail >= ctx->slot_count) {
        uint64_t new_tail = head - ctx->slot_count + 1;
    ...
    if (ctx->p_next_slot) {
        write_u64_be(ctx->p_next_slot, (uint64_t)ctx->next_free_slot);
    }
...
int32_t midware_shm_peek_latest_batch(midware_shm_ctx_t* ctx, midware_shm_batch_entry_t* entries,
    ...
    uint64_t available = head - tail;
    if (available > ctx->slot_count) available = ctx->slot_count;
    if (available > INDEX_QUEUE_SIZE) available = INDEX_QUEUE_SIZE;
    if (available > (uint64_t)max_entries) available = (uint64_t)max_entries;
...
int32_t midware_shm_peek_latest_packet(midware_shm_ctx_t* ctx, void* buf, int32_t max_len, midware_packet_header_t* out_header, const void** out_payload) {
    int32_t len = midware_shm_peek_latest(ctx, buf, max_len);
